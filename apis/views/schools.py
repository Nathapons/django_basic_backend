...
from ..serializers import SchoolHierarchySerializer, SchoolStructureSerializer, StudentSubjectsScoreDetailsSerializer


class StudentSubjectsScoreAPIView(APIView):

    def update_student_subject_score(self, instance):
        # Save Personnel
        if self.student_first_name or self.student_last_name:
            if self.student_first_name:
                instance.student.first_name = self.student_first_name

            if self.student_last_name:
                instance.student.last_name = self.student_last_name

            instance.student.save()

        # Save Subjects
        if self.subjects_title:
            instance.subjects.title = self.subjects_title
            instance.subjects.save()

        # Save StudentSubjectsScore
        if self.score:
    ...
    def create_student_subject_score(self):
        personnel = Personnel.objects.filter(
            first_name=self.student_first_name, 
            last_name=self.student_last_name
        ).last()
        subject = Subjects.objects.filter(title=self.subjects_title).last()

        stundent_subjects_score = StudentSubjectsScore.objects.create(
            student=personnel,
            subjects=subject,
            credit=self.credit,
            score=self.score
        )
        return stundent_subjects_score
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.student_first_name = request.data.get("first_name", None)
        self.student_last_name = request.data.get("last_name", None)
        self.subjects_title = request.data.get("subject_title", None)
        self.score = request.data.get("score", None)
        self.credit = request.data.get('credit', None)

        if not self.student_first_name:
            return Response({"message": "Request data doesn't has first_name"}, status=status.HTTP_400_BAD_REQUEST)

        if not self.student_last_name:
            return Response({"message": "Request data doesn't has last_name"}, status=status.HTTP_400_BAD_REQUEST)

        if not self.subjects_title:
            return Response({"message": "Request data doesn't has subject_title"}, status=status.HTTP_400_BAD_REQUEST)

        if self.score > 100 or self.score < 0:
            return Response({"message": "Score is not between 0 and 100"}, status=status.HTTP_400_BAD_REQUEST)

        if self.credit > 3 or self.credit < 0:
            return Response({"message": "Credit is not between 0 and 3"}, status=status.HTTP_400_BAD_REQUEST)

        student_subjects_scores = StudentSubjectsScore.objects.filter(
            student__first_name=self.student_first_name, 
            student__last_name=self.student_last_name,
            subjects__title=self.subjects_title,
            credit=self.credit
        )
        if not student_subjects_scores.exists():
            instance = self.create_student_subject_score()
        else:
            student_subjects_score = student_subjects_scores.last()
            instance = self.update_student_subject_score(student_subjects_score)
        ...
        return Response({"message": f"Create at id={instance.id} is successful"}, status=status.HTTP_201_CREATED)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        serializer = StudentSubjectsScoreDetailsSerializer(personnel)
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        personnels = Personnel.objects.filter(school_class__school__title=school_title)
        pattern = "{}. school: {}, role: {}, class: {}, name: {}"
        your_result = [
            pattern.format(
                personnel.id, 
                personnel.school_class.school.title, 
                personnel.get_personnel_type_display().replace("_", " ").capitalize(), 
                personnel.school_class.id,
                personnel.get_full_name()
            ) 
            for personnel in personnels
        ]

        return Response(data=your_result, status=status.HTTP_400_BAD_REQUEST)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        # Remark to P'Jo: Since database don't have teacher name, I will adjust new data_pattern instead
        '''
        data_pattern = [
            {
                "school": "school_name",
                "class1": [{"personnel_type": "student_name", .....}],
                "class2": [{"personnel_type": "student_name", .....}],
                "class3": [{"personnel_type": "student_name", .....}],
            },
            ....
        ]
        '''
        ...
        for school in schools:
            row.update({'school': school.title})
            school_classes = school.classes.all()

            for school_class in school_classes:
                class_name = f'class {school_class.class_order}'
                class_list = []

                teacher = Personnel.objects.filter(school_class__id=school_class.id, personnel_type=0).last()
                teacher_text = f'Teacher: {teacher.get_full_name()}'
                students = Personnel.objects.filter(school_class__id=school_class.id).exclude(personnel_type=0)
                for student in students:
                    full_name = student.get_full_name()
                    personnel_type_name = student.get_personnel_type_display()
                    class_list.append({personnel_type_name: full_name})

                row.update({class_name: {teacher_text: class_list}})
                class_list = []

            your_result.append(row)
            row = dict()

        return Response(data=your_result, status=status.HTTP_200_OK)
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        school_structures = SchoolStructure.objects.filter(parent__isnull=True)
        serializer = SchoolStructureSerializer(school_structures, many=True)
